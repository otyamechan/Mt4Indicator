...
extern bool MailMode = false;

extern bool SoundWarning = true;
extern int  SoundIntervalSec = 10;
extern string SoundFile = "alert2.wav";

extern bool UseLevelLine = true;
extern double AlertValue = 0.8;
//---- buffers
...
//---- indicators
   ...
   if(UseLevelLine){
      SetLevelValue(0,AlertValue);
      SetLevelValue(1,-AlertValue);
   }
//----
   return(0);
  }
...
//----
   ...
   static bool AlertEntry = false;
   if(ExtMapBuffer1[1] < AlertValue && ExtMapBuffer1[1] > -AlertValue )AlertEntry = false;
   if((ExtMapBuffer1[1] >= AlertValue || ExtMapBuffer1[1] <= -AlertValue ) && Time[1] !=alertTime && !AlertEntry){
      if(AlertMode) Alert("RCI Over  ["+DoubleToStr(ExtMapBuffer1[1],2)+"]    "+DoubleToStr(Bid,Digits)+" ["+TimeToStr(TimeLocal(),TIME_SECONDS)+"]");
      if(MailMode) SendMail("RCI Over "+Symbol()+Period(),"RCI Over  ["+DoubleToStr(ExtMapBuffer1[1],2)+"]    "+DoubleToStr(Bid,Digits)+" ["+TimeToStr(TimeLocal(),TIME_SECONDS)+"]");
      alertTime = Time[1];
      AlertEntry = true;
   }
   static datetime SoundAlertTime = 0;
   if((ExtMapBuffer1[1] >= AlertValue || ExtMapBuffer1[1] <= -AlertValue ) && TimeLocal()-SoundAlertTime > SoundIntervalSec ){
      if(SoundWarning) PlaySound(SoundFile);
      SoundAlertTime = TimeLocal();
   }
   return(0);
  }
//+------------------------------------------------------------------+