//------------------------------------------------------------------/
// Индикатор-советник, основаный на теории Демарка с дополнениями
// Часть кода взята из индикатора Ind-TD-DeMark-3-1.mq4 [Kara ©]
//------------------------------------------------------------------/
...
extern int showBars=200; // если = 0, то индюк отображается для всего графика
...
extern bool HorizontLine=true; // true = то будут прорисованы уровни пробития
...
extern bool Comments=true; // true = будут отображаться комментарии
extern int Trend=0; // 1 = только для UpTrendLines, -1 = только для DownTrendLines, 0 = для всех TrendLines

double Buf1[]; 
double Buf2[]; 

string Col[]={"Красная","Синяя","Розовая","Голубая","Коричневая","Салатная"};
      ...
      if (H1<0 || H2<0) Comm=Comm+"на графике недостаточно точек для построения\n";
         ...
         while(i>0 && isHitch==false){
            ...
            if (High[i]>tmp){
               ...
               if (qp!=""){
                  isHitch=true;
                  Text=Text+"Ист. "+DoubleToStr(tmp,qPoint)+" "+qp;
               ...
               else { Text=Text+"Лож. "+DoubleToStr(tmp,qPoint)+"; "; ObjectDelete(Rem);}
            }
            i--;
         }
         ...
         if (TakeLines){
            ...
            for(i=0;i<4;i++){
               ...
               Comm=Comm+DoubleToStr(qTL,qPoint)+" ("+DoubleToStr(HT[i]/Point,0)+"п.) ";
             }
             Comm=Comm+"\n";
         }
         else {
      ...
      if (L1<0 || L2<0) Comm=Comm+"на графике недостаточно точек для построения\n";
         ...
         if (TakeLines){ // линии целей
            ...
            for(i=0;i<4;i++){
               ...
               ObjectSet(Rem+i,OBJPROP_STYLE,STYLE_DOT);
               ObjectSet(Rem+i,OBJPROP_COLOR,col);
               Comm=Comm+DoubleToStr(qTL,qPoint)+" ("+DoubleToStr(LT[i]/Point,0)+"п.) ";
             }
            Comm=Comm+"\n";
         }
         else {
